    ...
    class BodyTemplates
    {
        static public Bone Human
        {
            get
            {
                ...
                Bone body =
                    new(BoneDef.Torso, new Sprite("bodyparts/chestbw") { OverlayName = "Shirt", OriginGround = new Vector2(5, distanceofbodyfromground) }, new Vector2(0), 0f,
                        new Bone(BoneDef.RightHand, new Vector2(-3, -21), -0.003f,
                            ...
                            new Bone(BoneDef.Hauled, new Vector2(-2, 11), 0.0005f) { RestingFrame = new Keyframe(10, Vector2.Zero, (float)Math.PI), SlotFunc = o => o.GetComponent<HaulComponent>().GetSlot() })
        ...
        static public Bone Skeleton
        {
            get
            {
                ...
                var spriteRibCage = new Sprite("mobs/skeleton/ribcage") { OverlayName = "Bone", OriginGround = new Vector2(8, 13) };               
                var spriteSkull = new Sprite("mobs/skeleton/skull") { OverlayName = "Bone", OriginGround = new Vector2(4, 8) };
                var spriteArm = new Sprite("mobs/skeleton/hand") { OverlayName = "Bone", OriginGround = new Vector2(2, 1) };
                var spriteLeg = new Sprite("mobs/skeleton/leg") { OverlayName = "Bone", OriginGround = new Vector2(3, 1) };

                Bone body =
                    new(BoneDef.Hips, spriteRibCage, Vector2.Zero, 0f,
                ...
                Bone root = new(BoneDef.Hips, spriteRibCage) { OriginGroundOffset = new Vector2(0, -11) };
                root.AddJoint(BoneDef.Head, new Joint(0, -12));
                root.AddJoint(BoneDef.RightHand, new Joint(-7, -7));
                root.AddJoint(BoneDef.LeftHand, new Joint(6, -8));
                root.AddJoint(BoneDef.RightFoot, new Joint(-3, 1));
                root.AddJoint(BoneDef.LeftFoot, new Joint(2, 0));

                Bone head = new(BoneDef.Head, spriteSkull) { Order = -.002f };
                Bone rhand = new(BoneDef.RightHand, spriteArm) { Order = -0.003f };
                Bone lhand = new(BoneDef.LeftHand, spriteArm) { Order = 0.002f };
                Bone rleg = new(BoneDef.RightFoot, spriteLeg) { Order = 0.001f };
                Bone lleg = new(BoneDef.LeftFoot, spriteLeg) { Order = 0.002f };

                rhand.AddJoint(BoneDef.Mainhand, new Joint(-2, 11) { Angle = 5 * (float)Math.PI / 4f, SlotGetter = (o) => GearComponent.GetSlot(o, GearType.Mainhand) });
