...
namespace Start_a_Town_.Components
{
    public class PersonalInventoryComponent : EntityComponent, IObjectSpace
    {
        ...
        readonly GameObjectSlot HaulSlot;// = new GameObjectSlot();
        ...
        {
            // don't i add these in the haulcomponent?
            ...
            //parent.AddAnimation(new Animation(AnimationDef.Haul).SetWeight(0)); 
        }
        ...
        public static void DropInventoryItem(GameObject parent, GameObject item)
        {
            ...
            slot.Clear();
            //parent.Net.Spawn(item, parent.Global + new Vector3(0, 0, parent.GetComponent<PhysicsComponent>().Height));
        ...
        public static void HaulFromInventory(GameObject parent, GameObject item, int amount = -1)
        {
            ...
            else if(amount < currentAmount)
            {
                ...
                if (server != null)
                {
                    ...
                    //parent.Net.InstantiateAndSync(splitItem);
            ...
            //slot.Object.StackSize -= amount == -1 ? currentAmount : amount;
            //parent.GetComponent<PersonalInventoryComponent>().Haul(parent, item);
            ////parent.Net.Spawn(item, parent.Global + new Vector3(0, 0, parent.GetComponent<PhysicsComponent>().Height));
        }
        ...
        internal override void HandleRemoteCall(GameObject parent, Message.Types type, BinaryReader r)
        {
            switch (type)
            {
                case Message.Types.Haul:
                    ...
                    //tohaulobj.StackSize = amount;
                    //if(tohaulobj.InstanceID != sourceobj.InstanceID)
                    if (hauledobj != null)
                    {
                        if (hauledobj.IDType == tohaulobj.IDType)
                        {
                            //var amount = tohaulobj.StackSize;
        ...
        bool PickUpOld(GameObject parent, GameObject obj)
        {
            if (obj == null)
                return true;
            var haulObj = this.HaulSlot.Object;
        ...
        bool PickUpOld(GameObject parent, GameObject obj, int amount)
        {
            ...
            //parent.Net.Spawn(newobj, parent, this.HaulSlot.ID);
            return true;
        }
        ...
        static public bool PickUpNew(GameObject parent, GameObject obj, int amount)
        {
            ...
            if (inv.HaulSlot.Object != null)
            {
                ...
                if (haulObj.IDType == obj.IDType)
                {
                    ...
                    (parent.Net as Net.Server).RemoteProcedureCall(new TargetArgs(parent), Message.Types.Haul, w => { w.Write(obj.RefID); w.Write(obj.RefID); });
                }
                //else
                // if we maxed out our hauling stack, but there is some remaining amout of item on the ground, store hauled stack and haul the remaining item stack
                if (!StoreHauled(parent))
        ...
        //static public void PickUpNewestBestest(GameObject parent, GameObject obj, int amount)
        //{
        //    var inv = parent.GetComponent<PersonalInventoryComponent>();
        //    var haulSlot = inv.HaulSlot;
        //    var haulObj = haulSlot.Object;
        //    if (haulObj != null)
        //    {
        //        if (haulObj.IDType == obj.IDType)
        //        {
        //            var transferAmount = amount == -1 ? obj.StackSize : amount;
        //            if (transferAmount > obj.StackSize)
        //                throw new Exception();
        //            if (transferAmount == 0)
        //                throw new Exception();
        ...
        //                server.RemoteProcedureCall(new TargetArgs(parent), Message.Types.Haul, w => { w.Write(newobj.InstanceID); w.Write(obj.InstanceID); w.Write(amount); });
        //            }
        //        }
        //    }
        //    return;
        ...
        static public bool PickUpNewNew(GameObject parent, GameObject obj, int amount)
        {
            ...
            else
            {
                ...
                {
                    var newobj = obj.Clone();// GameObject.Create(obj.IDType);
                    newobj.StackSize = amount;
                    parent.Net.InstantiateAndSync(newobj);
        ...
        public bool Haul(GameObject parent, GameObjectSlot objSlot)
        {
            ...
            //    ScreenManager.CurrentScreen.ToolManager.ActiveTool = new PlayerControl.HaulingTool();
            // else
