        ...
        public WorkbenchInterface(GameObject parent)
        {
            ...
            Button btn_craft = new Button(Panel_Selected.BottomLeft, panel_List.Width + Panel_Reagents.Width, "Craft")
            {
                LeftClickAction = () =>
                {
                    ...
                    Client.PlayerCraftRequest(new Components.Crafting.CraftOperation(this.SelectedReaction.ID, product.Requirements, this.BenchEntity, product.Tool, this.Bench.Storage));
                    return;
                }
            };
