...
{
    class SandboxClientEventHandler : EventHandlerClient
    {
                ...
                case Message.Types.EntityChangedChunk:
                    ...
                    foreach (var chunk in chunksToLoad)
                    {
                        ...
                        if (client.Map.GetActiveChunks().TryGetValue(chunk, out loaded))
                            ...
                            Network.Serialize(w => w.Write(chunk)).Send(Client.Instance.PacketID, PacketType.RequestChunk, Client.Instance.Host, Client.Instance.RemoteIP);
                    }
