...
namespace Start_a_Town_.Graphics
{
    public partial class Atlas : AtlasBase
    {
        ...
        {
            var tokens = new List<Node.Token>();
        ...
        public Node.Token Load(string assetPath, bool grayscale = false)
        {
            ...
            //this.Tokens.Add(assetPath, token);
        ...
        //readonly List<Node> Nodes = new() { new Node(0, 0, Size, Size) };// 2048, 2048) };
        ...
        public Vector2 Add2(Texture2D sprite)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                //{
                    ...
                    var bottom = new Node(new Rectangle(node.X, node.Y + sprite.Height, node.Width, node.Height - sprite.Height));
                    var top = new Node(new Rectangle(node.X + sprite.Width, node.Y, node.Width - sprite.Width, sprite.Height));
        ...
        public void Flush()
        {
            ...
            using var stream = new FileStream(GlobalVars.SaveDir + "atlas.png", FileMode.OpenOrCreate);
