...
namespace Start_a_Town_.UI
{
    public class ListCollapsibleNew : GroupBox
    {
        ...
        {
            this.ClearControls();
            //var /*queue*/ = new Queue<ListBoxCollapsibleNode>(new ListBoxCollapsibleNode[] { this.ARoot });
            var queue = new Queue<ListBoxCollapsibleNode>(this.ARoot.Children);

            while (queue.Any())
            {
                var node = queue.Dequeue();
                var nodeContainer = new GroupBox() { Name = "container", BackgroundColor = Color.Lime * .2f };
                var nodeItem = new GroupBox() { Name = "item" };// { BackgroundColor = Color.Yellow * .2f }; ;
                ...
                node.Parent?.ChildGroupBox.AddControlsBottomLeft(nodeContainer);
                ...
                void expand()
                {
                    if (!node.Expanded)
                    {
                        ...
                        nodeContainer.AddControlsBottomLeft(node.ChildGroupBox);
                        node.ChildGroupBox.Location.X = IndentWidth;
                    }
                    else
                    {
                        node.Expanded = false;
