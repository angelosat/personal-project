...
{
    class InteractionConstructOld : Interaction
    {
        ...
        public InteractionConstructOld(IConstructionProduct construction)
            : base("Construct", 1)
        {
            //this.Name = "Construct";
            //this.Seconds = 1;
        ...
        public bool DetectMaterialsOld(GameObject actor, TargetArgs t)
        {
            ...
            foreach (var req in reqs)
            {
                ...
                {
                    if (mat.GetInfo().ID == req.ObjectID)
                        req.AmountCurrent += mat.StackSize;
                }
                //return req.Amount >= req.Max;
                if (req.AmountCurrent < req.AmountRequired)
                    return false;
            }
            return true;
        }
        ...
        {
            return new InteractionConstructOld(this.Construction);
        }
    }
}
