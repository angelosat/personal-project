...
{
    class BlockBrowserNewNew : GroupBox// ScrollableBox
    {
        ...
        Panel Panel_Blocks;//, Panel_Variants;
                           //ButtonGridIcons<BlockRecipe.ProductMaterialPair> GridBlocks;
        UIToolsBox ToolBox;
        ...
        public BlockBrowserNewNew()
        {
            ...
            var blocks = Block.Registry.Values.Skip(1).ToList(); //skip air LOL FIX IT
            ...
            {
                //var list = cat.List.Select(r => r.Block).Where(b => b.Reagent?.Def.PreferredMaterialType != null);
                ...
                var grid = new ButtonGridIcons<Block>(4, 6, list, (slot, block) =>
                {
                    ...
                    slot.LeftClickAction = () =>
                    {
                        ...
                        this.ToolBox.SetProduct(product, OnToolSelected);
                            //var tools = this.CurrentSelected.Recipe.Category.GetAvailableTools(() => GetLastSelectedVariantOrDefault(this.CurrentSelected));
                    ...
                    //Right click to select variation", block.Name);// this.CurrentSelected.GetName());
        ...
        //private BlockRecipe.ProductMaterialPair GetLastSelectedVariantOrDefault(Block bl)
        //{
        //    return this.GetLastSelectedVariantOrDefault(bl);
        //}
        private BlockRecipe.ProductMaterialPair GetLastSelectedVariantOrDefault(Block block)
        {
            if (this.LastSelectedVariant.ContainsKey(block))
                ...
                //return new BlockRecipe.ProductMaterialPair(block, block.Reagent.Def.PreferredMaterialType.SubTypes.First()); // store last selected variant instead of getting first in list
                //return new BlockRecipe.ProductMaterialPair(block, block.DefaultMaterial); // store last selected variant instead of getting first in list
