...
namespace Start_a_Town_.Modules.Construction.UI
{
    class ConstructionsWindow : Window
    {
        ...
        readonly ListBox<BlockRecipe, Button> List_Costructions = new(new Rectangle(0, 0, 150, 200));
        readonly ListBox<BlockRecipe.ProductMaterialPair, Button> List_Variations = new(new Rectangle(0, 0, 150, 200));
        //readonly Block SelectedBlock;
        readonly Panel panel_Reagents;// = new Panel() { };
        readonly Panel panel_Selected;
        //readonly List<GameObjectSlot> matSlots;
        ...
        ConstructionsWindow()
        {
            ...
            var panel_filters = new Panel() { Location = this.Client.Controls.BottomLeft, AutoSize = true };
            panel_filters.Controls.Add(rd_All, rd_MatsReady);

            var panel_List = new Panel() { Location = panel_filters.BottomLeft, AutoSize = true };
        ...
        private static void BtnInitializer(BlockRecipe.ProductMaterialPair item, Button btn)
        {
            ...
            btn.LeftClickAction = () =>
            {
                ...
                ToolManager.SetTool(new ToolPlaceWall(Instance.PlayerConstructNew){Mode = item.Block.Multi ? ToolPlaceWall.Modes.Single : ToolPlaceWall.Modes.Wall});
        ...
        GroupBox Selected(BlockRecipe.ProductMaterialPair item)
        {
            ...
            var panelproduct = new PanelLabeled("Product") { AutoSize = true };
            var slot = new Slot<Cell>() { Location = panelproduct.Controls.BottomLeft };
            slot.Tag = new Cell() { Block = item.Block, BlockData = item.Data };
            var lbl = new Label(item.Block.Type.ToString()) { Location = slot.TopRight };
            panelproduct.Controls.Add(slot, lbl);

            var panelmats = new PanelLabeled("Materials") { AutoSize = true, Location = panelproduct.BottomLeft };
            var mat = new SlotWithText() { Tag = item.Req.ObjectID.GetObject().ToSlotLink(), Location = panelmats.Controls.BottomLeft };
        ...
        private void RefreshSelectedPanel(Panel panel_Selected, GameObject product)// Reaction reaction)
        {
            ...
            var tip = new CraftingTooltip(product.ToSlotLink(), new ItemRequirement(GameObject.Types.Campfire, 1));
        ...
        private void RefreshSelectedPanel(Panel panel_Selected, Reaction.Product.ProductMaterialPair product)// Reaction reaction)
        {
            ...
            var tip = new CraftingTooltip(product.Product.ToSlotLink(), product.Req);
            panel_Selected.Controls.Add(tip);
            return;
        }
        ...
        private void RefreshSelectedPanel(Panel panel_Selected, GameObject product, List<ItemRequirement> materials)// Reaction reaction)
        {
            ...
            var tip = new CraftingTooltip(product.ToSlotLink(), materials);
            panel_Selected.Controls.Add(tip);
            return;
        }
        ...
        void PlayerConstructNew(ToolPlaceWall.Args args)// Vector3 start, Vector3 end)
        {
            ...
            Net.Client.Instance.Send(PacketType.PlaceWallConstruction, data);
        }
    }
}
