    ...
    class StockpileUI : GroupBox
    {
        ...
        public StockpileUI(Stockpile stockpile)
        {
            ...
            var btnEdit = new IconButton()
            {
                ...
                LeftClickAction = () => ScreenManager.CurrentScreen.ToolManager.ActiveTool = new ToolDesignatePositions(this.Edit, () => stockpile.Positions.Select(g => (Vector3)g - Vector3.UnitZ).ToList()) { ValidityCheck = IsPositionValid }// this.Create)
            };
        ...
        private void Delete()
        {
            Client.Instance.Send(PacketType.StockpileDelete, PacketStockpileDelete.Write(PlayerOld.Actor.RefID, this.Stockpile.ID));
        }
        ...
        public new void Refresh()
        {
            ...
            var list = from obj in this.Stockpile.GetContents() select obj.ToSlotLink();
        ...
        private void SelectFilter(string filter, bool value)
        {
            ...
            p.Add(filter, value);
            Client.Instance.Send(PacketType.StockpileFilters, Network.Serialize(p.Write));
        }
        ...
        private void Refresh(Dictionary<GameObject, int> addedobjects, Dictionary<GameObject, int> removedobjects)
        {
            ...
            {
                var label = this.TableContents.GetElement(i => i.Key.IDType == item.Key.IDType, "Amount") as Label;
        ...
        internal static Window ShowWindow(Stockpile s)
        {
            var win = GetWindow(s);
            win.ToggleSmart();
            return win;
