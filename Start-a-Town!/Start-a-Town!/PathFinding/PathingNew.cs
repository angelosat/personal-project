...
namespace Start_a_Town_.PathFinding
{
    public class PathingNew
    {
        ...
        const float CostStraight = 10, CostDiag = 14, CostClimb = 140;
        //public class Path
        //{
        ...
        public void Begin(IMap map, Vector3 start, Vector3 finish)
        {
            ...
            this.State = PathingNew.States.Working;
            this.PathInProgress = new Path();
            var startNode = new Node(this.Map, this.Start, this.Finish, GetCostEuclidean);
            var finishNode = new Node(this.Map, this.Finish, this.Finish, GetCostEuclidean);
            this.Nodes.Clear();
            this.Open.Clear();
            this.Closed.Clear();
        ...
        public void Work()
        {
            ...
            if (this.Open.Count > 0)
            {
                ...
                //if (current.Global == this.Finish)
                {
                    ...
                    // NO i do that at the aimoveto behavior
                ...
                foreach (var n in neighbors) // put visibility check here
                {
                    ...
                    {
                        nnode = new Node(this.Map, n, this.Finish, GetCostEuclidean);
                        this.Nodes[n] = nnode;
                    }
        ...
        static float Heuristic(Vector3 start, Vector3 finish)
        {
            ...
            //var dz = (float)Math.Max(0, finish.Z - start.Z); // no cost if target is lower
            return (dmax - dmin) * CostStraight + dmin * CostDiag;// +dz * CostClimb;
        }
        static float HeuristicOld(Vector3 start, Vector3 finish)
        {
            var dx = (float)Math.Abs(finish.X - start.X);
            var dy = (float)Math.Abs(finish.Y - start.Y);
            //var dz = (float)Math.Abs(finish.Z - start.Z);
        ...
        {
            //if (!IsWalkable(map, next.Global))//current.Global))
           ...
           // ComputeCost(current, next);
            ...
            if (next.CostFromStart < costOld)
            {
                ...
                this.Open.Enqueue(next.CostFromStart + Vector3.DistanceSquared(current.Global, next.Global), next);
            }
        }
        ...
        static void ComputeCost(Node current, Node next)
        {
            ...
            var costNew = current.CostFromStart + cost;
            if (costNew < next.CostFromStart)
            {
                next.Parent = current;
                next.CostFromStart = costNew;
            }
        }
        ...
        static void ComputeCostTheta(NodeBase current, NodeBase next)
        {
            ...
            if (los)
            {
                cost = GetCostEuclidean(current.Parent, next);
            ...
            {
                cost = GetCostEuclidean(current, next);
        ...
        static float GetCost(Node current, Node next)
        {
            //return Heuristic(current.Global, next.Global);
            ...
            if (current.Global.X == next.Global.X || current.Global.Y == next.Global.Y)
                ...
                cost = CostDiag;
            if (next.Global.Z != current.Global.Z)
                cost += CostClimb;
            return cost;
        }
        static float GetCostEuclidean(NodeBase current, NodeBase next)
        {
            //var h = Heuristic(current.Global, next.Global);
            //return h;
        ...
        static bool IsWalkable(IMap map, Vector3 global)
        {
            if (!map.IsSolid(global - Vector3.UnitZ))
                return false;
            //if (map.IsSolid(global) && map.IsSolid(global + Vector3.UnitZ))
            //    return false;
        ...
        static bool CanWalkOnBlock(IMap map, Vector3 global)
        {
            ...
            //    return false;
            if (map.IsSolid(global + Vector3.UnitZ) && map.IsSolid(global + Vector3.UnitZ + Vector3.UnitZ))
                return false;
            return true;
        }
        ...
        public static void HandleInaccessibleBlock(IMap map, Vector3 global)
        {
            if (IsInaccessible(map, global))
                return;
            var area = FloodFill(map, global); // maybe spread this over multiple frames or put it on another thread?
            // maybe add specific global to a temporary list while performing floodfill over multiple frames? (so other agents can ignore it)
    ...
    class Parameters
        {
            public static readonly Parameters Default = new Parameters(true);
            public bool AllowFalling = false;// true;
            public Parameters(bool allowFalling)
            {
