...
namespace Start_a_Town_.Blocks.Smeltery
{
    partial class BlockSmeltery
    {
        public class Entity : BlockEntityWorkstation// BlockEntity, IBlockEntityWorkstation
        {
            ...
            public Start_a_Town_.Components.Crafting.CraftOperation SelectedProduct;

            public override Container Input { get { return this.Storage; } }
            public Container Storage, Output, Fuels;

            public Entity(int inCapacity, int outCapacity, int fuelCapacity)
            {
                ...
                this.Fuels = new Container(fuelCapacity) { Name = "Fuel", Filter = item => item.Body.Material.Fuel > 0 };// MaterialsComponent.IsFuel(item) };
            ...
            {
                var children = this.Storage.Slots.Concat(this.Output.Slots).Concat(this.Fuels.Slots).ToList();
                return children;
            }
            ...
            internal override void HandleRemoteCall(IMap map, Vector3 global, ObjectEventArgs e)
            {
                ...
                switch (e.Type)
                {
                    ...
                    case Message.Types.Craft:
                        e.Data.Translate(net, r =>
                        {
                            this.Craft(map.GetNetwork().GetNetworkObject(r.ReadInt32()), r.ReadVector3(), new CraftOperation(net, r));
                        });
                        break;
            ...
            private void Craft(GameObject actor, Vector3 global, CraftOperation crafting)
            {
                var workstation = actor.Map.GetBlockEntity(global) as BlockSmeltery.Entity;
            ...
            private bool MaterialsAvailable()
            {
                foreach (var mat in this.SelectedProduct.Materials)//.Requirements)
                    if (this.Storage.Slots.GetAmount(obj => obj.GetID() == mat.ObjectID) < mat.Max)
                        return false;
                return true;
            }
            ...
            private void ConsumeFuel()
            {
                ...
                if (found == null)
                    return;
                this.Power.Max = 0;
                foreach (var p in from p in found.Object.GetComponent<MaterialsComponent>().Parts.Values select p.Material.Fuel)
            ...
            private void Finish(IMap map, Vector3 global)// GameObject parent)
            {
                ...
                var existing = (from slot in this.Output.GetNonEmpty()
                                where slot.Object.ID == product.ID
                ...
                if (empty != null)
                {
                    ...
                    var data = RPCargs.Create(new TargetArgs(global), Message.Types.AddProduct, w => w.Write(product.InstanceID));// product.Write(w));
            ...
            {
                return new Entity(this.Storage.Capacity, this.Output.Capacity, this.Fuels.Capacity);
            }
            ...
            //    tag.Add(new SaveTag(SaveTag.Types.Compound, "Output", this.Output.Save()));
            //    tag.Add(new SaveTag(SaveTag.Types.Compound, "Fuel", this.Fuels.Save()));
            //    return tag;
            //}
            //public override void Load(SaveTag save)
            //{
            //    save.TryGetTag("Input", t => this.Input.Load(t));
            //    save.TryGetTag("Output", t => this.Output.Load(t));
            //    save.TryGetTag("Fuel", t => this.Fuels.Load(t));
            //}
            ...
            public override SaveTag Save(string name)
            {
                ...
                tag.Add(new SaveTag(SaveTag.Types.Compound, "Input", this.Storage.Save()));
                tag.Add(new SaveTag(SaveTag.Types.Compound, "Output", this.Output.Save()));
                tag.Add(new SaveTag(SaveTag.Types.Compound, "Fuel", this.Fuels.Save()));
            ...
            public override void Load(SaveTag tag)
            {
                ...
                //    {
                //        //this.PendingOrders.Enqueue(new CraftOperation(order));
                //        this.PendingOrders.Add(new CraftOrder(order));
                //    }
                //});

                tag.TryGetTag("Input", t => this.Storage.Load(t));
                tag.TryGetTag("Output", t => this.Output.Load(t));
                tag.TryGetTag("Fuel", t => this.Fuels.Load(t));
            }
            ...
            public override void Read(BinaryReader r)
            {
                ...
                //    this.PendingOrders.Add(new CraftOrder(r));
                //}
            ...
            class InteractionCraft : Interaction
            {
                ...
                BlockSmeltery.Entity Workbench;

                public InteractionCraft(Reaction.Product.ProductMaterialPair product, BlockSmeltery.Entity entity, Vector3 workstationGlobal)
                    : base(
                    "Craft",
                    1
                        //,
                ...
                public override void Perform(GameObject actor, TargetArgs target)
                {
                    ...
                    (actor.Map.GetBlockEntity(target.Global) as Entity).Out(this.Product.Product);
