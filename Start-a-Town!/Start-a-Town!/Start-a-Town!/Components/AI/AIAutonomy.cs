    ...
    class AIAutonomy : Behavior
    {
        ...
        public override string Name
        {
            get
            {
               ...
               // foreach (Interaction i in PlanInteractions)
                foreach (Queue<InteractionOld> goal in CurrentPlans)
                    foreach (InteractionOld i in goal)
                        text += i + "\n";
                return text.TrimEnd('\n');
            }
        }
        ...
        public override BehaviorState Execute(GameObject parent, AIState state)//Net.IObjectProvider net, GameObject parent, Personality personality, Knowledge knowledge, params object[] p)
        {
            ...
            if (Timer <= 0)
            {
                ...
                {
                    Queue<InteractionOld> newGoal = new Queue<InteractionOld>();
                    ...
                    {
                        Condition searchCond = new Condition((actor, target) => true, "I need " + need.Name.ToLower() + ".",
                            new Precondition(need.Name, i => true, PlanType.FindNearest),
                            new Precondition(
                                need.Name,
                                i => ProductionComponent.CanProduce(i.Source, product => InteractionOld.HasInteraction(product, parent, inter => inter.NeedEffects.Any(n => n.Name == need.Name))),// inter.Effect.Key == need.Name)),
