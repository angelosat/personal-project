...
namespace Start_a_Town_.Components
{
    class AttackComponent : Component
    {
        ...
        public int NearestEnemyUpdateTimerMax
        {
            get
            {
                return (int)(NearestEnemyIntervalSeconds * Engine.TargetFps);
            }
        }
        ...
        public void Finish(GameObject parent)
        {
            ...
            //Animation.Start(parent, coll, 1f, onFinish: new Dictionary<Bone.Types, Action>() { { Bone.Types.Torso, () =>
            //{
        ...
        {
            //Perform(parent, parent.Direction, attack);
            //parent.Global.ToConsole();

            this.Body.FadeOutAnimation(this.AniAttack);
            //this.Body.FadeOutAnimation(AnimationCollection.DeliverAttack);
        ...
        void Perform(GameObject actor, Vector3 direction, Attack attack)
        {
            ...
            var attackOrigin = actor.Global + actor.GetPhysics().Height * .5f * Vector3.UnitZ;
            ...
            foreach (var obj in actor.GetNearbyObjects(range: (r) => r <= range))
                ...
                // check line of sight between actor attack origin (half his height), and the END POINT OF HIS ATTACK!!! NOT WORLD POSITION OF COLLIDING ENTITY!!!! FFS
                    // BUT WHY NOT?
                {
                    attackEndPoint = obj.Global + obj.GetPhysics().Height * .5f * Vector3.UnitZ;
        ...
        static GameObject FindClosestEnemy(GameObject parent)
        {
            ...
            list = list.Where(foo => ResourcesComponent.HasResource(foo, Resource.Types.Health)).ToList();
