...
namespace Start_a_Town_
{
    ...
    public class Blueprint
    {
        ...
        static public GameObject Create(GameObject.Types blueprintIndex, GameObject.Types productIndex, params BlueprintStage[] stages)
        {
            ...
            obj["Info"] = new GeneralComponent(blueprintIndex, ObjectType.Blueprint, "Blueprint: " + GameObject.Objects[productIndex].Name, "A blueprint");
         ...
         //   obj["Interactive"] = new InteractiveComponent(Ability.GetAbilityObject(Script.Types.CraftingWorkbench));
            obj.AddComponent<InteractiveComponent>().Initialize(Script.Types.CraftingWorkbench);
            GameObject.Objects[blueprintIndex] = obj;
            return obj;
        }
        ...
        static public bool Copy(Net.IObjectProvider net, GameObject.Types bpID, GameObject materialsParent, ItemContainer materialsContainer, GameObject productsParent, ItemContainer targetContainer, out GameObject product)
        {
            ...
            if (firstAvailable.IsNull()) // if no available slot, pop loot it
            {
        ...
        public bool Craft(Net.IObjectProvider net, GameObject materialsParent, ItemContainer materialsContainer, GameObject productsParent, ItemContainer targetContainer, out GameObject product)
        {
            // check materials
            if (!CheckMaterials(materialsContainer))
            {
            ...
            GameObjectSlot firstAvailable = (from slot in targetContainer
                                             where slot.HasValue
                                             where slot.Object.ID == ProductID
                                             where slot.StackSize < slot.StackMax
                                             select slot)
                                             .FirstOrDefault();
            if(firstAvailable.IsNull())
        ...
        public static bool CheckMaterials(GameObjectSlotCollection container, Dictionary<GameObject.Types, int> mats)
        {
            foreach (var mat in mats)
            {
                ...
                container
                    .FindAll(slot => slot.HasValue)
                    .FindAll(slot => slot.Object.ID == mat.Key)
        ...
        void RemoveMaterials(Net.IObjectProvider net, GameObjectSlotCollection container)
        {
            foreach (var stage in this.Stages)
                foreach (var mat in stage)
                {
        ...
        static void RemoveMaterials(Net.IObjectProvider net, GameObjectSlotCollection container, Dictionary<GameObject.Types, int> mats)
        {
            foreach (var mat in mats)
            {
                int remaining = mat.Value;
    ...
    class Plan
    {
        ...
        {
            GameObject obj = new GameObject();
            obj["Info"] = new GeneralComponent(blueprintIndex, ObjectType.Plan, "Plan: " + GameObject.Objects[productIndex].Name, "A plan");
            obj["Gui"] = GameObject.Objects[productIndex]["Gui"];
            obj["Physics"] = new PhysicsComponent();
            obj["Sprite"] = new SpriteComponent(Map.ItemSheet, new Rectangle[][] { new Rectangle[] { Map.Icons[24] } }, new Vector2(16, 16));
            obj["Blueprint"] = new BlueprintComponent().SetValue("Blueprint", new Blueprint(productIndex, stages));
            //obj["Equip"] = new EquipComponent(Stat.Mainhand.Name);//, 0);
            obj.AddComponent<EquipComponent>().Initialize(GearType.Mainhand);
          //  obj["Abilities"] = new FunctionComponent(Ability.Attack, Ability.Construct);

            obj["Use"] = new UseComponentOld()
            {
                Use = (a) =>
                {
                    throw new NotImplementedException();
                    //a.Target.PostMessage(Message.Types.SetBlueprint, a.Actor, productIndex);
                }
            };
    ...
    class Schematic
    {
        ...
        {
            GameObject obj = new GameObject();
            obj["Info"] = new GeneralComponent(blueprintIndex, ObjectType.Schematic, "Schematic: " + GameObject.Objects[productIndex].Name, "A schematic");
            obj["Gui"] = GameObject.Objects[productIndex]["Gui"];
            obj["Physics"] = new PhysicsComponent();
            obj["Sprite"] = new SpriteComponent(Sprite.Default);//Map.ItemSheet, new Rectangle[][] { new Rectangle[] { Map.Icons[24] } }, new Vector2(16, 16));
            obj["Blueprint"] = new BlueprintComponent().SetValue("Blueprint", new Blueprint(productIndex, stages));
