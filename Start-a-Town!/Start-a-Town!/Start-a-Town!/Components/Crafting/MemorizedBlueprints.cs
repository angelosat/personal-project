    ...
    class MemorizedBlueprints
    {
        ...
        public void Gain(GameObject crafter, GameObject bp, int value)
        {
            BlueprintMemory memory = (from mem in this.InnerList
                                      ...
                                      where mem.Blueprint == bp.ID
                                      select mem).FirstOrDefault();
            if (memory.IsNull())
            {
                memory = new BlueprintMemory(bp.ID);
                this.InnerList.Add(memory);
            }
            int intellect = crafter.GetComponent<AttributesComponent>().GetAttribute(Attribute.Types.Intelligence).Value; StatsComponent.GetStat(crafter, Attribute.Intelligence.Name);
