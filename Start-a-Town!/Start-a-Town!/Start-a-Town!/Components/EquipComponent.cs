...
namespace Start_a_Town_.Components
{
    class EquipComponent : Component
    {
        ...
        public Resource Durability { get { return (Durability)this["Durability"]; } set { this["Durability"] = value; } }
        ...
        public EquipComponent()
        {
            ...
            this.Type = null;
            this.Durability = Resource.Create(Resource.Types.Durability, 20, 20);// new Durability() { Max = 20, Value = 20 };
        ...
        public override object Clone()
        {
            ...
            //EquipComponent phys = new EquipComponent().Initialize(this.Slot, this.Abilities.ToArray());
            EquipComponent phys = new EquipComponent().Initialize(this.Type, this.Abilities, this.Stats);
            //phys.Durability = this.Durability.Clone() as Resource;
            phys.Durability = Resource.Create(this.Durability);
        ...
        public override void Query(GameObject parent, List<InteractionOld> list)//GameObjectEventArgs e)
        {
            ...
            list.Add(new InteractionOld(new TimeSpan(0, 0, 0, 0, 500), Message.Types.Equip, name: "Equip",
                range: (actor, target) =>
                {
                    return target.Exists ? InteractionOld.DefaultRangeCheck(actor, target) : InventoryComponent.HasObject(actor, foo => foo == target);
                }, //range: -1,
        ...
        public override void GetPlayerActionsWorld(GameObject parent, Dictionary<PlayerInput, Interaction> list)
        {
            ...
            list.Add(PlayerInput.ActivateHold, new Interactions.Equip());// new Equip());

        }
        internal override List<SaveTag> Save()
        {
         //   return new List<SaveTag>() { new SaveTag(SaveTag.Types.Compound, "Durability", this.Durability.Save()) };
            var list = new List<SaveTag>() { this.Durability.Save("Durability") };
            return list;
        }
        internal override void Load(SaveTag save)
        {
            save.TryGetTag("Durability", tag => this.Durability = Resource.Create(tag));
        }
