...
{
    class InventoryComponent : Component, IObjectSpace
    {
       ...
       // Dictionary<string, ItemContainer> Containers = new Dictionary<string,ItemContainer>();// { get { return (Dictionary<string, ItemContainer>)this["Containers"]; } set { this["Containers"] = value; } }
        ...
        bool Hold(Net.IObjectProvider net, GameObject parent, GameObjectSlot objSlot)
        {
            if (objSlot == null)
                return true;
            if (!objSlot.HasValue)
            ...
            foreach (var container in Containers)
                foreach (GameObjectSlot invSlot in container)
                    if (invSlot.Object != null)
                        if ((invSlot.Object as GameObject).ID == objID)
                        {
                            slot = invSlot;
                            return true;
                        }
        ...
        static public bool GiveObject(Net.IObjectProvider net, GameObject receiver, GameObject obj, int amount = 1)
        {
            InventoryComponent inv;
        ...
        private static void Hold(Net.IObjectProvider net, GameObject receiver, GameObject obj)
        {
            if (obj.IsNull())
                return;
            if (receiver == obj)
