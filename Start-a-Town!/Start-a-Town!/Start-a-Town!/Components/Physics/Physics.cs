...
namespace Start_a_Town_.Components.Physics
{
    class Physics
    {
        ...
        static public void Update(float weight, float friction, IMap map, Vector3 global, Vector3 velocity, out Vector3 nextglobal, out Vector3 nextvelocity)
        {
            ...
            Block block = thisCell.Block;// Block.Registry[thisCell.Block.Type];
            //float density = 1 - (block.IsSolid(map, global) ? block.Density : 0); //net
            float density = 1 - (block.IsSolid(thisCell, global.ToBlock()) ? block.Density : 0); //net
