    ...
    class WorkbenchInterface : GroupBox
    {
        ...
        public WorkbenchInterface(GameObject parent)
        {
            ...
            Button btn_craft = new Button(Panel_Selected.BottomLeft, panel_List.Width + Panel_Reagents.Width, "Craft")
            {
                LeftClickAction = () =>
                {
                    ...
                    Net.Client.PlayerCraftRequest(new Components.Crafting.Crafting(this.SelectedReaction.ID, product.Requirements, this.BenchEntity, product.Tool));
                    return;

                    GameObject prod = Panel_Selected.Tag as GameObject;

                    // check materials and if they don't exist return
                    foreach(var mat in Slots_Reagents)
                    {
                        if (mat.Object == null)
                            return;
                        if (!this.Bench.Slots.Check(mat.Object.ID, mat.Object.StackSize))
                            return;
                    }

                    var p = this.SelectedReaction.Products.First().GetProduct(this.SelectedReaction, parent, Slots_Reagents);
        ...
        private List<Reaction> GetAvailableBlueprints()
        {
            return (from reaction in Reaction.Dictionary.Values
                    ...
                    where reaction.Sites.Contains(this.BenchEntity.GetInfo().ID)//(int)parent.ID)
