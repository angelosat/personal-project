...
namespace Start_a_Town_
{
    ...
    {
        public enum Messages { Default, Remove, MouseUp }

        public event EventHandler<EventArgs> MouseLeft, Removed;
        ...
        public virtual Messages OnMouseLeft(bool held)
        {
            if (MouseLeft != null)
        ...
        public virtual void Update()
        {
            ...
            //var closestTarget = this.GetClosestObject();
            //if(Camera.BlockTargeting && closestTarget != null)
            //    this.Target = closestTarget;

            //this.Target = Camera.BlockTargeting ?  : Controller.Instance.Mouseover.Target;

            //this.Target = (this.BlockTargeting ? Controller.Instance.Mouseover.Target : Controller.Instance.MouseoverEntity.Target) ?? TargetArgs.Empty;
            //this.Target = (this.BlockTargeting ? Controller.GetMouseoverBlock() : Controller.GetMouseoverEntityPriority()) ?? TargetArgs.Empty;
            //this.Target = (InputState.IsKeyDown(Keys.LMenu) ? Controller.GetMouseoverBlock() : Controller.GetMouseoverEntityPriority()) ?? TargetArgs.Empty;

            if ((Controller.Instance.Mouseover.Object is UI.Element))
                 this.Target = TargetArgs.Empty;
            else
                this.Target = Controller.Instance.Mouseover.Target ?? TargetArgs.Empty;
            //else
            ...
            //    if (Net.Client.Instance.Map.GetBlock(this.Target.Global).Type == Block.Types.Air)
            //        throw new Exception();
        }
        ...
        TargetArgs GetClosestObject()
        {
            ...
            if (!Player.Actor.Exists)
                return null;
            var closest = Player.Actor.GetNearbyObjects(r => r <= Components.InteractionOld.DefaultRange).OrderBy(o=>Vector3.DistanceSquared(o.Global, Player.Actor.Global)).FirstOrDefault();
        ...
        internal virtual void HandleContextMenuSelection(Components.InteractionOld inter) { }
        ...
        //public virtual void DrawBlockHighlight(SpriteBatch sb, IMap map, Camera camera)
        //{
        //    if (TargetOld.IsNull())
        //        return;
        ...
        //        return;
        //    //Sprite sprite = Block.TileSprites[Block.Types.Sand];//Target.GetSpriteOrDefault(); //
        //    float cd;
        ...
        //    int x = (int)Math.Abs(Face.X), y = (int)Math.Abs(Face.Y), r = (int)camera.Rotation;
        //    int z = (int)Math.Abs(Face.Z);
        //    int highlightIndex = (2 * z) + (1 - z) * ((x + r) % 2);
        //    int frontback = InputState.IsKeyDown(Keys.RShiftKey) ? 1 : 0;
        //    cd = (TargetOld.Global + Face).GetDrawDepth(Engine.Map, camera);
        ...
        internal virtual void DrawAfterWorld(MySpriteBatch sb, IMap map, Camera camera)
        {
