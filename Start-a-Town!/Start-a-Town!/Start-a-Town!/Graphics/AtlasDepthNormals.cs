...
{
    public partial class AtlasDepthNormals : IAtlas
    {
        ...
        public Node.Token Load(string assetName, string depthMap)
        {
            ...
            //if (!ok)
            //(token.DepthTexture as RenderTarget2D).ContentLost += AtlasDepthNormals_ContentLost;
            //ok = true;
        ...
        //bool ok;
        //void AtlasDepthNormals_ContentLost(object sender, EventArgs e)
        //{
        //    "event".ToConsole();
        //}
        ...
        public Node AddTexture(Node.Token token, out Rectangle rect)
        {
            ...
            //emptyNode.NormalTexture = token.NormalTexture;
        ...
        public void OnDeviceLost()
        {
            ...
            {
                token.OnDeviceReset();
                //if ((token.DepthTexture as RenderTarget2D).IsContentLost)
                //    throw new Exception();
            }
            this.Bake();
        }
        ...
        internal void Bake()
        {
            ...
            RenderTarget2D normaltexture = new RenderTarget2D(gfx, Size, Size);
            ...
            foreach (var node in nodes)
            {
                ...
                //if ((node.DepthTexture as RenderTarget2D).IsContentLost)
                //    throw new Exception();
            ...
            foreach (var node in nodes)
            {
                //if (node.NormalTexture.IsNull())
                if (node.CurrentToken == null)
                    continue;
                Vector2 loc = new Vector2(node.Rectangle.X, node.Rectangle.Y);
                sb.Draw(node.NormalTexture, loc, Color.White);
                if (node.NormalTexture.IsDisposed)
                    "ASDSAD".ToConsole();
            }
            sb.End();
            //BakeTexture(gfx, node.DepthTexture, depthtexture, ref sb, ref nodes);
            //BakeTexture(gfx, node.NormalTexture, normaltexture, ref sb, ref nodes);
        ...
        //private void BakeTexture(GraphicsDevice gfx, Texture2D sourceTexture, RenderTarget2D destTexture, ref SpriteBatch sb, ref List<Node> nodes)
        //{
        //    gfx.SetRenderTarget(destTexture);
        //    gfx.Clear(Color.Transparent);
        //    sb = new SpriteBatch(gfx);
        //    sb.Begin();
        //    nodes = this.RootNode.GetChildren();
        ...
        //            continue;
        //        Vector2 loc = new Vector2(node.Rectangle.X, node.Rectangle.Y);
        //        sb.Draw(sourceTexture, loc, Color.White);
