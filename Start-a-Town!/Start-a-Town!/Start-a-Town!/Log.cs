...
namespace Start_a_Town_
{
    ...
    public class Log
    {
        static Queue<Entry> EntryQueue = new Queue<Entry>();
        public enum EntryTypes { Damage, Death, Equip, Unequip, Default, TooHeavy, Buff, Debuff, Skill, CellChanged, 
            /// <summary>
            ...
            public override string ToString()
            {
                ...
                switch (Type)
                {
                    ...
                    case EntryTypes.Chat:
                        ...
                        return "[" + (Values[0].IsNull() ? "" : (Values[0] as GameObject).Name) + "] " + Values[1];
...
");
        ...
        internal static void Command(EntryTypes type, params object[] p)
        {
            ...
            switch (type)
            {
                ...
                case EntryTypes.DialogueOption:
                    ...
                    //GameObject.PostMessage(target, Message.Types.DialogueOption, speaker, option);
                    break;
                ...
                switch (msg)
                {
                    ...
                    default:
                       // Log.Write(EntryTypes.Default, "Invalid Command");
                        //Log.Help();
                        Log.Enqueue(Log.EntryTypes.Chat, Player.Actor, gotText);
                        SpeechBubbleOld.Create(Player.Actor, gotText);
        ...
        {
            Player.Actor.PostMessage(Message.Types.ChunkLoaded);
        }
        ...
        private static void Give(Queue<string> words)
        {
            try
            {
                ...
                InventoryComponent.Give(target, obj.ID, amount);
        ...
        private static void Set(Queue<string> words)
        {
            ...
            switch (setWhat)
            {
                ...
                case "skill":
                    ...
                    if (!Player.Actor["Skills"].Properties.ContainsKey(skillName))
                        Log.Write(EntryTypes.Default, "Invalid Skill");
                    float old = Player.Actor["Skills"].GetProperty<float>(skillName);
                    Player.Actor["Skills"][skillName] = value;
                    Log.Write(EntryTypes.Skill, skillName + " set to " + value);
                    break;
                case "hour":
                case "time":
                    //Map.Instance.DayTime = float.Parse(words[2]);
                    int t = int.Parse(words.Dequeue());//words[2]);
                    //Map.Instance.AddTime(t);
                    Engine.Map.Time = new TimeSpan(Engine.Map.Time.Days, t, Engine.Map.Time.Minutes, Engine.Map.Time.Seconds);
                    ...
                    try
                    {
                        ...
                        switch (words.Count)
                        {
                            ...
                            case 4:
                                ...
                                needType = (Need.Types)Int32.Parse(need);
                                Player.Actor["Needs"].GetProperty<NeedsCollection>("Needs")[needType].Value = value;
                                break;
                            default:
                                break;
                        }
