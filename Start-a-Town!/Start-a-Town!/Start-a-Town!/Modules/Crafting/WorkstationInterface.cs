...
{
    class WorkstationInterface : GroupBox
    {
        ...
        public Action<CraftOperation> CraftCallback;

        Panel Panel_bplist, Panel_blueprint, Panel_Selected;
        ReagentPanel Panel_Reagents;

        public WorkstationInterface(BlockEntity parent, IsWorkstation.Types type, Vector3 workstationGlobal, List<GameObjectSlot> reagentSources)
        {
            ...
            Button btn_craft = new Button(Panel_Selected.BottomLeft, panel_List.Width + Panel_Reagents.Width, "Craft")
            {
                LeftClickAction = () =>
                {
                    ...
                    //Net.Client.PlayerCraftRequest(new Components.Crafting.CraftOperation(this.SelectedReaction.ID, product.Requirements, null, product.Tool, this.Bench.Storage));

                    var op = new Components.Crafting.CraftOperation(this.SelectedReaction.ID, product.Requirements, this.WorkstationGlobal);
                    if (this.CraftCallback != null)
                        this.CraftCallback(op);
                    else
                        Net.Client.PlayerCraftRequest(op);
                    return;
                }
            };
