...
namespace Start_a_Town_
{
    struct QueueEnrty
    {
        ...
        {
            return (string)Map["Name"] + " " + ChunkCoords.ToString();
        }
    ...
    public class ChunkLoader
    {
        ...
        {
            Chunk cached;
            return this.TryEnqueue(request, callback, out cached);
        }
        ...
        BlockingCollection<ChunkRequest> Requests = new BlockingCollection<ChunkRequest>(new ConcurrentQueue<ChunkRequest>());
        BlockingCollection<Chunk> ToSave = new BlockingCollection<Chunk>(new ConcurrentQueue<Chunk>());
        Dictionary<Vector2, Chunk> Cache = new Dictionary<Vector2, Chunk>();
        public ConcurrentQueue<Chunk> Output = new ConcurrentQueue<Chunk>();
        IMap Map;
        static public ChunkLoader StartNew(IMap map)
        {
            ChunkLoader loader = new ChunkLoader(map);
            loader.Start();
            return loader;
        }
        void Start()
        ...
        public Chunk Generate(Vector2 chunkvector)
        {
            ...
            newChunk.InitCells(this.Map.GetWorld().GetMutators().ToList());//.FinalizeCells(Net.Server.Random); // WARNING!
            ...
            this.Map.GetWorld().GetMutators().ToList().ForEach(m => m.Finally(newChunk));
        ...
        public Chunk LoadChunk(Map map, Vector2 chunkLocalCoords, LightingEngine engine)
        {
            ...
            else
            {
                ...
                newChunk.InitCells(map.World.Mutators.ToList());//.FinalizeCells(Net.Server.Random); // WARNING!
                ...
                do
                {
                    ...
                    if (ChunksToLoad.Count > 0)
                    {
                        ...
                        while (ChunksToLoad.Count > 0)
                        {
                            ...
                            QueueEnrty entry;
                            if (!ChunksToLoad.TryDequeue(out entry))
                                continue;
                    ...
                    if (ChunksToUnload.Count > 0)
                    {
                       ...
                       // ChunksInMemory[Engine.Map].Remove(pos);
                        Chunk ch;
                        ChunksInMemory[Engine.Map].TryRemove(pos, out ch);
                    }
        ...
        private void OnChunkReady(IMap map, Vector2 chunkCoords, Chunk chunk)
        {
            ...
            //    return;
            Dictionary<Vector2, Queue<ChunkHandler>> callbackQueues;
            if (!CallBackEvents.TryGetValue(map, out callbackQueues))
                return;
        ...
        Dictionary<IMap, Dictionary<Vector2, Queue<ChunkHandler>>> CallBackEvents;
        ConcurrentQueue<QueueEnrty> ChunksToLoad;
        ConcurrentStack<ConcurrentQueue<QueueEnrty>> MapsToLoad;
        Queue<Vector2> ChunksToUnload;
        ...
        static public bool TryGetChunk(Map map, Vector2 chunkCoords, out Chunk chunk)
        {
            ...
            ConcurrentDictionary<Vector2, Chunk> chunks;
            if (!Instance.ChunksInMemory.TryGetValue(map, out chunks))
            {
                chunk = null;
                return false;
            }
        ...
        ChunkLoader()
        {
            ...
            Thread = new Thread(new ParameterizedThreadStart(AsyncLoadChunks));
        ...
        //static public void LoadAsync(Map map, Vector2 chunkCoords, CancellationToken cancelToken, Net.IObjectProvider net, Action<Chunk> onChunkInit = null)
        //{
        //    Instance.ChunksInMemory.AddOrUpdate(map, new ConcurrentDictionary<Vector2, Chunk>(), (m, dic) => dic);
        //    Task.Factory.StartNew(() =>
        //    {
        ...
        public Chunk LoadChunk(IMap map, Vector2 chunkLocalCoords)
        {
            ...
            else
            {
                ...
                newChunk.InitCells(map.GetWorld().GetMutators().ToList());//.FinalizeCells(Net.Server.Random); // WARNING!
                newChunk.ResetHeightMap();
                map.GetWorld().GetMutators().ToList().ForEach(m => m.Finally(newChunk));
                //newChunk.ResetEdges();
