        ...
        public override void Finally(Chunk chunk)
        {
            ...
            foreach (var worm in worms)
            {
                ...
                foreach (var segment in segments)
                {
                    ...
                    var chunkbox = new BoundingBox(new Vector3(chunkglobal, 0), new Vector3(chunkglobal + (Chunk.Size - 1) * Vector2.One, Map.MaxHeight - 1));
            ...
            for (int i = x; i <= xx; i++)
                for (int j = y; j <= yy; j++)
                    for (int k = z; k <= zz; k++)
                    {
                        ...
                        var c = chunk.CellGrid2[Chunk.FindIndex(local)];
