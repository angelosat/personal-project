...
namespace Start_a_Town_.UI
{
    class LobbyWindow : Window
    {
           ...
           // Players.Build(Start_a_Town_.Client.Players, ip => ip.Name);// ip.ToString());
            ...
            Txt_Input = new TextBox(Vector2.Zero, new Vector2(Panel_Chat.ClientSize.Width, TextBox.DefaultHeight))
            {
                ...
                EnterFunc = (e) =>
                {
                    ...
                    //{
                    //    writer.Write(Net.Client.PlayerData.Name.Length);
                    //    writer.Write(Encoding.ASCII.GetBytes(Net.Client.PlayerData.Name));
                    //    writer.Write(Txt_Input.Text.Length);
                    //    writer.Write(Encoding.ASCII.GetBytes(Txt_Input.Text));
                    //});
                    //Net.Client.Send(Packet.Create(PacketType.Chat, data));
            ...
            //Server.OnPlayerConnect += new EventHandler(Server_OnPlayerConnect);
            //Server.OnPlayersChanged += new EventHandler(Server_OnPlayerConnect);
        ...
        {
            Instance.Players.Build(Net.Client.Players.GetList(), ip => ip.Name);
        }
    }
}
