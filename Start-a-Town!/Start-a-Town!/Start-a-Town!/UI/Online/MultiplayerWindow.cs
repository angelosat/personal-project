...
namespace Start_a_Town_.UI
{
    class MultiplayerWindow : Window
    {
        ...
        {
            if (this.Character.Character.IsNull())
                return false;
        ...
        private void Host()
        {
            ...
            Net.Client.Connect(localHost, new PlayerData(Txt_Name.Text), a => { LobbyWindow.Instance.Console.Write("Connected to " + localHost); });
                    ...
                    LeftClickAction = () =>
                    {
                     //   ManualResetEvent manual = new ManualResetEvent(false);
                        ...
                        try
                        {
                            ...
                            Net.Client.Connect(address, new PlayerData(Txt_Name.Text), ar =>
                            {
                                //Rooms.Ingame ingame = new Rooms.Ingame();
                                //ScreenManager.Add(ingame.Initialize());
                            });
                            //Net.Client.EnterWorld(this.Character.Character);
                        }
                        catch (Exception e)
                        {
                            // COULD NOT CONNECT
                            ScreenManager.Remove();
                            //UI.MessageBox.Create(e.GetType().ToString(), e.Message, "Back", () => { }).ShowDialog();
                            //UI.MessageBox.Create(e.GetType().ToString(), e.Message, "Back", () => { }).ShowDialog();
                     ...
                     //   manual.WaitOne();
                        ...
                        //Engine.PlayGame(SelectCharacterWindow.GetLastChar());
                        //Engine.PlayGame(this.Character.Character);
                        window.Hide();
                    }
                });
