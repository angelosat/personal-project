...
{
    class NewWorldWindow : Window
    {
        #region Singleton
        static NewWorldWindow _Instance;
        public static NewWorldWindow Instance
        {
            get
            {
                if (_Instance == null)
                    _Instance = new NewWorldWindow();
                return _Instance;
            }
        }
        ...
        //static public event EventHandler WorldCreated;
        //void OnWorldCreated()
        //{
        //    if (WorldCreated != null)
        //        WorldCreated(this, EventArgs.Empty);
        //}
        ...
        NewWorldWindow()
        {
          ...
          //  Chk_Caves = new CheckBox("Caves", new Vector2(Txt_Seed.Left, Txt_Seed.Bottom));
            ...
            //var list = Terraformer.All;
            //Combo_Terraformers = new ComboBox<Terraformer>(
            //    new ListBox<Terraformer, Button>(new Rectangle(0, 0, 150, Button.DefaultHeight * list.Count)).Build(list, t => t.Name),
            //    t => t.Name
            //    ) { Location = Chk_Trees.BottomLeft, Text = "Select Terraformer" };



            Tab_World.Controls.Add(lbl_name, Txt_MapName, lbl_seed, Txt_Seed, btn_random, Chk_Trees);//, Combo_Terraformers); //Chk_Flat,Chk_Caves,
        ...
        void tab_Click(object sender, EventArgs e)
        {
            ...
            Control panel = (sender as Control).Tag as Control;
            if(panel == null)
                return;
        ...
        void btn_Create_Click(object sender, EventArgs e)
        {
            ...
            //world = World.Create(new WorldArgs(Txt_MapName.Text, Chk_Flat.Checked, Chk_Caves.Checked, Chk_Trees.Checked, seed, Chk_Flat.Checked ? Terraformer.Flat : Terraformer.Normal));
            world = World.Create(new WorldArgs(Txt_MapName.Text, Chk_Trees.Checked, seed, mutators)); //Chk_Caves.Checked,
        ...
        {
            //Chunk chunk = ChunkLoader.Demand(map, Vector2.Zero);
            Chunk chunk = ChunkLoader.Load(map, Vector2.Zero);
            map.AddChunk(chunk);
            //chunk.UpdateEdges(map);
            LightingEngine engine = new LightingEngine(map)
            {
                OutdoorBlockHandler = (ch, cell) =>
                {
                    ...
                    //Cell.UpdateEdges(map, cell.GetGlobalCoords(ch), Edges.All, VerticalEdges.All);
                }
            };
            var positions = chunk.ResetHeightMap();
            engine.HandleBatchSync(positions);
            //engine.Enqueue(positions);
            //engine.HandleBatchSync();



            //engine.HandleSkySync();
        }
