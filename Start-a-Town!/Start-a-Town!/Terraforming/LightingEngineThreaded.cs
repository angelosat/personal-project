...
namespace Start_a_Town_
{
    public class LightingEngineThreaded
    {
       ...
       // public BlockingCollection<Vector3> SkyLight { get; set; }
        ...
        public LightingEngineThreaded(IMap map)
            : this()
        {
            this.Map = map;
        }
        ...
        public LightingEngineThreaded(IMap map, Action<IEnumerable<Vector3>> batchFinishedCallback, Action<IEnumerable<Vector3>> blockCallback)
            : this()
        {
            //this.Net = net;
            this.Map = map;
        ...
        static public LightingEngineThreaded StartNew(IMap map, Action<IEnumerable<Vector3>> lightCallback, Action<IEnumerable<Vector3>> blockCallback)
        {
            //return new LightingEngineThreaded(map, lightCallback, blockCallback);//.Start();

            return new LightingEngineThreaded(map, lightCallback, blockCallback).Start();
        }
        public LightingEngineThreaded Start()
        {
            Task.Run(() =>
            {
                //try { HandleSkyAsync(); }
        ...
        public void HandleBatchSync(IEnumerable<Vector3> vectors)
        {
            ...
            if (skydeltas.Count > 0)
                 //(this.Map.Net.ToString() + " skylight changed").ToConsole();
        ...
        void HandleSkyGlobal(Vector3 global, Queue<Vector3> queue, Dictionary<Vector3, byte> deltas)
        {
            ...
            //if (global == new Vector3(15, 18, 65))
            //    "gamimeno5".ToConsole();
