    ...
    public class UIContextActions : Panel// GroupBox
    {
        ...
        public void RefreshOld()
        {
            ...
            if (target == null)
                target = TargetArgs.Null;
            this.Controls.Clear();
            var interactions = target.GetContextActionsWorld(PlayerOld.Actor);
        ...
        public void RefreshOlder()
        {
            ...
            if (target == null)
                ...
                target = TargetArgs.Null;
            this.Controls.Clear();
            var interactions = target.GetContextActionsWorld(PlayerOld.Actor);// Rooms.Ingame.GetNet());//Client.Instance);
            if (interactions != null)
                foreach (var i in interactions)
                {
                    var available = i.Value.AvailabilityCondition(PlayerOld.Actor, target);
                    //if (available || (!available && this.ShowUnavailableActions))
            ...
            foreach(var input in PlayerInput.DefaultInputs)
            {
                ...
                var i = input.Value(PlayerOld.Actor, target);
                if (i == null)
                    continue;
                var available = i.AvailabilityCondition(PlayerOld.Actor, target);
                if (available || (!available && this.ShowUnavailableActions))
                    this.Controls.Add(new Label(input.Key.ToString() + ": " + i.Name) { Location = this.Controls.BottomLeft, MouseThrough = true, TextColorFunc = () => available ? Color.White : Color.Maroon });
            }
