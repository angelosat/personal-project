...
namespace Start_a_Town_.UI
{
    class MessageBox : Window
    {
        ...
        MessageBox(string title, string text, string b1text, Action b1action)
        {
            ...
            int width = 600;// this.ClientSize.Width;
        ...
        private void CreateButtonsVer(ContextAction[] actions, int width)
        {
            foreach (var action in actions)
            {
                Button btn = new Button(action.Name(), width) { LeftClickAction = () => { action.Action(); this.Hide(); } };
                //btn.Location = this.Panel_Buttons.Controls.BottomLeft;
                btn.Location = this.Panel_Buttons.Controls.TopRight;
                //this.Panel_Buttons.Controls.Add(btn);
                this.Panel_Buttons.AddControlsBottomLeft(btn);
            }
        }
