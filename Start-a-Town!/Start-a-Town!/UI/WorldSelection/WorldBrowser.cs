...
{
    class WorldBrowser : GroupBox
    {
        ListBox<DirectoryInfo, Button> List_Worlds;
        Action<IWorld> Callback;
        public WorldBrowser(int width, int height, Action<IWorld> callback)
        {
        ...
        public void Refresh()
        {
            DirectoryInfo[] worlds = World.GetWorlds();
        ...
        void SelectWorld(DirectoryInfo worldDir)
        {
            ...
            World world = World.Load(worldDirectory);
            this.Callback(world);
        }
    }
}
