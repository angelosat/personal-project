...
{
    class WorldScreenUI : Control
    {
        #region Singleton
        static WorldScreenUI _Instance;
        public static WorldScreenUI Instance
        {
            get
            {
                if (_Instance == null)
                    _Instance = new WorldScreenUI();
                return _Instance;
            }
        }
        #endregion
        ...
        Button 
            //Btn_Play, 
            Btn_EnterMap, Btn_NewWorld, Btn_Refresh, Btn_WorldRename, Btn_Back, Btn_WorldList;
        ...
        readonly CharacterBrowser CharacterBrowser;
        ...
        WorldScreenUI()
        {
            ...
            //Panel_MapInfo = new Panel(new Vector2(WindowManager.LastScreenWidth,0), new Vector2(200, 500));
            ...
            //Panel_WorldButtons.Location.Y = WindowManager.LastScreenHeight - Panel_WorldButtons.Height;
            Panel_WorldButtons.Location.Y = UIManager.Height - Panel_WorldButtons.Height;

            Panel_WorldList = new Panel() { AutoSize = true };
            //WorldBrowser = new WorldBrowser(200, WindowManager.LastScreenHeight - Panel_Info.Height - Panel_WorldButtons.Height, Load);
            WorldBrowser = new WorldBrowser(200, UIManager.Height - Panel_Info.Height - Panel_WorldButtons.Height, Load);
            ...
            {
                HoverFunc = () => (Window_Character.Tag as GameObject) is null ? "No character selected" : ""
            };
            ...
            this.MapPopUp = new UI.MapPopUp(m => EnterMap(m));// { BtnEnter.HoverFunc = () => (Window_Character.Tag as GameObject).IsNull() ? "No character selected!" : "" };
            this.MapPopUp.BtnEnter.HoverFunc = () => (Window_Character.Tag as GameObject) is null ? "No character selected!" : "";

            //CheckBox platebox = Nameplate.CheckBox;
            //platebox.Location = Panel_Info.TopRight;

            this.Controls.Add(Panel_Info, Panel_WorldButtons
                //, platebox
                ); //, Panel_MapInfo
        ...
        private void InitCharacterPanel()
        {
            ...
            var btn_select = new Button("Select Character", Window_Character.Client.Width)
            {
            ...
            var btn_create = new Button("Create New Character", Window_Character.Client.Width)
            {
                Location = btn_select.Location,
                Anchor = Vector2.UnitY,
                LeftClickAction = () =>
                {
                    var win = new NewCharacterWindow();
                    win.HideAction = () =>
                    {
        ...
        {
            NewWorldWindow.Instance.Callback = this.Load;
        ...
        void EnterMap(IMap map)
        {
            ...
            if (Window_Character.Tag is not GameObject character)
                return;
            ...
            ScreenManager.Add(Rooms.Ingame.Instance.Initialize(null));

            //ScreenManager.Add(new Rooms.Ingame());
        ...
        void EnterMap()
        {
            if (Panel_MapInfo.Tag is not Map map)
                return;
            //XDocument xml = Engine.Settings.ToXDocument();
            //var profile = xml.Root.Element("Profile");
            //if (profile.IsNull())
            ...
            Engine.Config.GetOrCreateElement("Profile").GetOrCreateElement("LastWorld").Value = map.World.Name;
        ...
        public override void Reposition(Vector2 ratio)
        {
            ...
            //this.Panel_MapInfo.Height = this.WindowManager.LastScreenHeight - this.Window_Character.Height;
