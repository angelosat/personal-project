...
float4 GetFogColor(float z)
{
	...
	if (FogZ > 1)
		if (z < FogZ + FogLevel)
		{
			...
			d = clamp(d, 0, FogDistance) / FogDistance;
			float4 finalFogColor = float4(d, d, d, d);// fog.r, fog.g, fog.b, d);// val);
...
MyVertexOutput MyChunkVertexShader(MyVertexInput input)
{
	...
	bool visible = input.BlockCoords.z <= MaxDrawLevel
		//&& input.BlockCoords.z > FogZ // simple bottleneck testing?
		;
...
//	clip(color.r == 1 && color.g == 0 && color.b == 0 ? -1 : 1); // i dont remember why i put that here, but now i dont want it to clip if i write a red color (like a entity flashing red when it gets attacked)
	...
	//float a = input.Color.a;
	//return (1-a)*color + float4(a*color.rgb*input.Color.rgb, color.a);
	// i moved those two above between the unused calculations
...
float4 mixWithMaterial(float4 color, float4 material)
{
	...
	//not this one//float4 outcol = shininess; // OMG!!! COOL EFFECT FOR GHOSTS (because of the alpha)
	//this one//return pixelshine;// OMG!!! COOL EFFECT FOR GHOSTS (because of the alpha)
...
float4 MapDump(VertexShaderOutput input, out float depthBuffer : DEPTH0, out float4 fogoutput : COLOR1) : COLOR0
{
	...
	float d = depth.r;
	depthBuffer = d;
	fogoutput = float4(fog.r, 0, 0, 1); //float4(fog.r, 0, 0, 1);
	return color;
}
...
MyPixelOutput MyChunkPixelShader(MyVertexOutput input)
{
	...
	if (HideWalls && PlayerOcclusion)
		if (input.BlockCoords.z >= PlayerGlobal.z)
		{
			...
			{
				float2 positionNormalised = input.ScreenPosition.xy / ViewportW;// float2(1, (Viewport.x / Viewport.y));
				 positionNormalised += float2(-PlayerCenterOffset.x,PlayerCenterOffset.y);// float2(1, (Viewport.x / Viewport.y));
	...
	output.Color = input.Color * color.a * mixWithMaterial(color, input.Material);// *(1 - occlusion);
	output.Depth = depthToWrite + float4(0, 0, 0, 1);
	output.DepthBuffer = depthToWrite;
